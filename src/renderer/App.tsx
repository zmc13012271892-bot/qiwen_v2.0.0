    ...
    try {
      if (isRegMode) {
        const result = await dispatch(registerUser({
          email: regForm.email, username: regForm.username,
          password: regForm.password, displayName: regForm.displayName || regForm.username,
        }));
        if (registerUser.rejected.match(result)) throw new Error(result.error?.message || '注册失败');
      } else {
        const emailVal = (csEmailRef.current?.value || '').trim();
        const pwdVal = csPwdRef.current?.value || '';
        const result = await dispatch(loginUser({ emailOrUsername: emailVal, password: pwdVal, rememberMe: true }));
        if (loginUser.rejected.match(result)) throw new Error(result.error?.message || '登录失败');
      }
      setShowLogin(false);
